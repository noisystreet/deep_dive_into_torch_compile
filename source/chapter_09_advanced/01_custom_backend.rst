...
后端抽象层的设计用意是将"编译器实现"与"编译器调用"解耦。torch.compile 只负责"什么时机编译"（Dynamo 的 guard 机制）和"编译什么"（AOTAutograd 生成的 FX Graph），不关心"怎么编译"。这意味着任何一个能够消费 FX Graph 的编译器，无论它是用 Triton、MLIR、TVM 还是手写汇编，都可以作为 torch.compile 的后端。这种设计的核心价值在于：**用户不需要改变调用方式（仍然是 ``@torch.compile`` ），只需要切换后端名称，就能获得完全不同的编译策略和硬件支持**。
...
- 自定义后端 **没有自动微分功能** 。传入后端的图是 AOTAutograd 分区后的，前向子图不包含反向信息。如果后端需要自己处理微分，需要使用 AOTAutograd 级别的接口。这意味着不能简单地在自定义后端中调用 ``.backward()``——你接收到的前向子图只是一个前向计算，不包含任何梯度信息。如果需要支持训练，必须通过 AOTAutograd 接口获取 joint graph，自己处理图分区和自动微分。
- 自定义后端 **不参与算子分解** 。传入的图中可能包含高层算子（如 ``layer_norm`` ）。后端需要自己处理这些算子。这是后端设计中最容易被忽视的问题：Inductor 对 ``layer_norm`` 的分解发生在 AOTAutograd 之后、Inductor 自身的 lowering 之前。自定义后端跳过了 Inductor 的预处理流程，因此收到的图中包含的是高层算子而非分解后的基本算子。如果后端不支持这些高层算子，必须在前置阶段自行分解，否则只能回退到 eager 执行——这实际上抵消了编译带来的大部分性能优势。
- 自定义后端生成的代码 **不受 PyTorch 的 guard 保护** 。如果输入形状变化导致重新编译，后端会再次被调用。这意味着自定义后端需要自己管理编译缓存和 guard 逻辑。Inductor 内置了多层缓存（FxGraphCache、Triton 磁盘缓存、PyCodeCache），在形状变化时通过 ``max_fusion_size`` 和 grid size 调整来避免重新编译。自定义后端默认没有这些机制，每次形状变化都会触发完全重新编译，可能导致编译时间大幅增加。
