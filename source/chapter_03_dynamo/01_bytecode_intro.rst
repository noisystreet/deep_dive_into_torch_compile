...
Dynamo 选择了第三条路。关键 invariant 是：**graph break 不是 bug，是设计特性** （第 1.1 节、第 3.6 节）。这直接体现了第 2.1 节的「编译器适应 Python」原则。
...
.. code-block:: text
   ...
   字节码符号执行（InstructionTranslator）
       ...
       ├─ FakeTensor：提供 shape/dtype，不分配显存（第 3.4 节）
       └─ Proxy / VariableTracker：在 FX Graph 里占位（第 3.4 节）
       │
       ▼
   Guard 树（CheckFunctionManager）
       │  问题：编译结果能否用于这次输入？
       └─ 编译期记录条件，运行期 O(1) 检查（第 3.5 节）
       │
       ▼
   缓存 + Graph Break
       │  问题：何时复用、何时重编译、何时放弃编译？
       ├─ code object 缓存链表（第 3.7 节）
       └─ 无法捕获 → 断图，子图分别编译（第 3.6 节）

**代价与边界** 。这套设计不是免费的：

- graph break 过多 → kernel launch 碎片化，加速比下降。
- guard 过细 → 形状略变就重编译，编译时间压过收益（第 3.8 节讨论符号形状作为缓解）。
