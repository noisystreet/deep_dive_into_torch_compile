...
对照 PyTorch v2.12.1 源码时，建议从 `ir.py <file:///workspace/pytorch/torch/_inductor/ir.py>`__ 顶部的 ``[Note: Inductor IR]`` （159–198 行）读起——那是官方对整套 IR 设计意图的浓缩说明。
...
Inductor IR **故意不** 表达 block size、mask、shared memory——这些在 `codegen/triton.py <file:///workspace/pytorch/torch/_inductor/codegen/triton.py>`__ 的 ``TritonKernel`` 里生成：
